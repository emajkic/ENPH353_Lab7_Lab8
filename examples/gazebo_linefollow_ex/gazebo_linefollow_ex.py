...
if __name__ == '__main__':
    ...
    qlearn.loadQ("Current_best_policy.pkl")
    ...
    for x in range(total_episodes):
        ...
        while True:
            ...
            try:
                # Some Gazebo gym environments return camera image directly
                if isinstance(observation, dict) and 'camera' in observation:
                    img = observation['camera']
                elif isinstance(observation, (list, tuple)) and hasattr(observation[0], 'shape') and len(observation[0].shape) == 3:
                    img = observation[0]
                else:
                    img = None
                ...
                if img is not None:
                    ...
                    # Overlay info text on the image
                    info_text = f"Ep:{x}  Step:{i}  R:{reward:.2f}  Eps:{qlearn.epsilon:.3f}  Act:{action}"
                    cv2.putText(img, info_text, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 
                    0.6, (255, 255, 255), 2)
        ...
        if highest_reward < cumulated_reward:
                ...
                # Save policy with highest reward yet
